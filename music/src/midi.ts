...
export interface MidiNote {
  /** MIDI pitch (0-127). For drums, use GM_DRUMS constants */
  pitch: number
  ...
  /** Velocity (0-127) */
  velocity: number
  /** MIDI channel (0-15, default 0; drums = 9) */
  channel?: number
}
...
export function onsetsToNotes(
  ...
  pitch: number,
  options?: { duration?: number; channel?: number }
): MidiNote[] {
...
function writeVarLen(value: number): number[] {
  if (value < 0) throw new Error('Negative var-length value')
  const bytes: number[] = []
 ...
 */
export function createMidiBuffer(options: CreateMidiOptions): Buffer {
  ...
  for (const track of tracks) {
    ...
    for (const note of sorted) {
      ...
      timedEvents.push(
        { tick: startTick, data: [0x90 | ch, note.pitch, vel] },
        { tick: endTick, data: [0x80 | ch, note.pitch, 0] }
      )
    }
 ...
 */
export async function writeMidiFile(
  path: string,
  options: CreateMidiOptions
): Promise<string> {
 ...
 */
export async function writeOnsetsMidi(
  path: string,
  onsets: Array<{ time: number; velocity?: number }>,
  options: {
    tempo: number
