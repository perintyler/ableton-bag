 ...
 */
export async function findDemucs() {
 ...
 */
export async function separate(options) {
    const { input, outputDir, model = 'htdemucs', twoStems, device = 'cpu', python, } = options;
    const { python: resolvedPython, available } = python
        ? { python, available: true }
        : await findDemucs();
    if (!available) {
        throw new Error('Demucs not found. Install it with: python3 -m pip install demucs soundfile');
    }
...
`;
    const { stdout, stderr } = await exec(resolvedPython, ['-c', script, input, outputDir], { timeout: 600_000 });
    // Parse the JSON output from the last line
    const lines = stdout.trim().split('\n');
    const jsonLine = lines[lines.length - 1];
    const stemPaths = JSON.parse(jsonLine);
    return {
        model,
